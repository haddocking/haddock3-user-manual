...
- `hydrogen_build`: Use this parameter to control which hydrogen atoms will be (re)built (`all` (default) or only `unknown`). The `unknown` option might be useful for small ligands, or very large systems in case where the molecule has already passed through HADDOCK and contains all hydrogens).
