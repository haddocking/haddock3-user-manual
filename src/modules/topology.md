...
It will:
- Build the CG structure from the AA, based on the MARTINI mapping (`.pdb`)
...
Therefore, in a CG docking pipeline, it is necessary to run this module after the `[topoaa]` module (second step of the workflow).

Note that currently the module does not support non-standard biomolecules (apart from standard amino-acids, DNA, RNA), such as small-molecules and coarbohydrates. Those are ignored in CG mode, but will re-appear if present in the initial AA structure after the back-mapping stage (`cgtoaa`) (provided parameters and topologies for the ligand have been provided or automatically generated).
...
- `cgffversion`: defines the force-field version for the CG mapping and parameters. Currently on MARTINI2 is supported.
