...
The `[cgtoaa]` module performs the backmapping of the CG representation to AA. 
The back-mapping procedure uses the distance restraints (tables *_cg_to_aa.tbl) built `[topocg]` module. 
During the initial CG mapping stage in `[topocg]`, atoms-to-bead restraints are defined, consisting of one distance restraint per bead 0Å (distance defined as 0) between the geometric center of the atoms and the bead to which they belong. 
...
- (i) An initial fitting of the single AA structures onto their CG counterparts using the atoms-to-bead distance restraints, downscaling the intermolecular interactions to avoid clashes between molecules;  
...
- (iii) final clash removal through additional rounds of minimization and brief MD. 

During all those steps, the CG model is kept rigid and fixed in space, and both AA and CG models co-exist, but don’t see each other except through the effect of the CG to AA distance restraints.
