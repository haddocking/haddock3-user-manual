...
`[gdock]` has a narrower scope than `[rigidbody]`: it only supports the 20 standard amino acids, and only two-body docking. Restraints are limited to plain ambiguous/unambiguous distance restraints; center of mass restraints, symmetry restraints, and other NMR-derived restraint types (RDCs, PCSs, etc.) supported by `[rigidbody]` are not available in `[gdock]`.
